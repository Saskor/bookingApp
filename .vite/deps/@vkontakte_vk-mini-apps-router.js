import {
  bridge
} from "./chunk-RCEI74MP.js";
import {
  Link
} from "./chunk-OEKW6VBW.js";
import "./chunk-OMDFP43T.js";
import {
  require_jsx_runtime
} from "./chunk-RJDO2IQI.js";
import {
  require_react
} from "./chunk-OLG33HYM.js";
import {
  __toESM
...
function fillParamsIntoPath(path, params) {
  ...
  const paramInjector = (acc, param) => {
    ...
    invariant2(params?.[paramName], `Missing parameter ${paramName} while building route ${path}`);
...
function buildPanelPathFromModalMatch(match, router) {
  ...
  function rateByParams(route2) {
    return Object.keys(match.params).map((key) => Number(Boolean(route2.path?.includes(`:${key}`)))).reduce((acc, item) => acc + item, 0);
  }
...
var InitialLocation = class _InitialLocation {
  ...
  static get value() {
    return _InitialLocation._instance?._value;
  }
};
...
function RouterProvider({ router, children, notFound, hierarchy, notFoundRedirectPath, interval = 400, useBridge = true, throttled = true }) {
  ...
  const isPopoutShown2 = router.state.location.state?.[STATE_KEY_SHOW_POPOUT];
...
function useActiveVkuiLocation() {
  ...
  const route = match?.route;
  const modal = state.location.state?.[STATE_KEY_SHOW_MODAL] ?? (route && "modal" in route ? route.modal : void 0);
  return {
